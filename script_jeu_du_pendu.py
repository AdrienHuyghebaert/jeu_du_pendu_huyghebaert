...
# chaine de caractères en un type liste pour faciliter l'analyse.
...
# Cette procédure propose à l'utilisateur de rejouer ou non au jeu.
...
def donner_indice(liste_utilisateur_testees, mot_liste, alphabet_liste):
    ...
    # Soustraire les lettres déjà utilisées (données par l'utilisateur ou bien constituants le mot)
...
# CECI EST LA FONCTION PRINCIPALE QUI APPELLE LES AUTRES FONCTIONS ET PROCEDURES.
def jeu_du_pendu():
    faire_introduction()
    ...
    utilisation_indice = 1  # Compteur qui permet de savoir combien de fois, l'indice a été utilisé.
    indice = []  # Initialisation de la variable indice.

    # Boucle pour les différentes tentatives jusqu'à que l'un ou l'autre des critères soit remplis.
    # Soit l'utilisateur n'a plus d'essais disponible et il a perdu.
    # Soit l'utilisateur n'a plus de lettres à trouver et il a donc gagner.
    while lettres_a_trouver != 0 and nombre_essais != 0:
        ...
        # Si l'utilisateur n'a pas trouvé de lettre.
            ...
            # Ajout de la lettre qui vient d'être testée aux autres
